...
public class CustomKeywordsForEnquiry {
	...
	public static void fillDtlsOfFareEnqWithClusterStn(String trainNo,
			String journeyDate,
			String srcCode,
			String destnStnCode,
			String clsCode,
			...
			String returnClass)
	{
		...
		WebUI.click(findTestObject('Object Repository/NxtGenPRS_OR/Enquiry/FareEnq_B3/div_ClusterClass'))


		for (int classCount = GlobalVariable.classCountStart; classCount <= GlobalVariable.maxClusterCls; classCount++) {

			testObject = 'Object Repository/NxtGenPRS_OR/Enquiry/FareEnq_B3/clusterClass'+ classCount
			...
			if (WebUI.verifyElementPresent(findTestObject(testObject), GlobalVariable.waitForElementPresentSec, FailureHandling.OPTIONAL) == true) {
				...
				//	System.out.println('CLASS NAME IS=' + className)
				guiClassCode = className.split('-')[0].trim()
				System.out.println(' GUI Cluster CLass CLASS CODE IS=' + guiClassCode)
				System.out.println('INPUT Cluster CLASS CODE IS=' + clusterClass)
				if (guiClassCode.equals(clusterClass)) {
					System.out.println('CLICKING Cluster CLASS CODE')
					WebUI.click(findTestObject(testObject))
